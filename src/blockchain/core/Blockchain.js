...
const GenesisBlock_1 = require("../config/GenesisBlock");
const Transaction_1 = require("../models/Transaction");
const KeyManager_1 = require("../crypto/KeyManager");
const TokenConfig_1 = require("../../economy/TokenConfig");
const MiningPool_1 = require("../../consensus/MiningPool"); // NEW: Mining Pool
/**
...
class Blockchain extends events_1.EventEmitter {
    constructor(genesisValidatorId, validatorPool) {
        super();
        this.recentTxIds = new Set();
        this.RECENT_TX_CACHE_SIZE = 1000;
        this.chain = [];
        this.state = new Map();
        this.genesisValidatorId = genesisValidatorId;
        this.validatorPool = validatorPool;
        this.miningPool = new MiningPool_1.MiningPool(); // Initialize mining pool
        this.initializeGenesis();
    }
    /**
     * DEV ONLY: Force set account state (for Relayer/Testing)
     */
    forceSetAccountState(address, accountState) {
        this.state.set(address, accountState);
    }
    /**
     * Cache recent transaction IDs and prune old ones
     */
    cacheTransactionId(txId) {
     ...
     */
    isTransactionDuplicate(txId) {
        // 1. Check recent cache
        if (this.recentTxIds.has(txId))
            return true;
    ...
    initializeGenesis() {
        ...
        const genesisBlock = new Block_1.Block(GenesisBlock_1.GENESIS_BLOCK_DATA);
     ...
     */
    addBlock(transactions, validatorId, signature, timestamp, signatures) {
        ...
        const newBlock = Block_1.Block.create(nextIndex, previousHash, transactions, validatorId, stateRoot, undefined, // node_wallet
        ...
        const totalBlockFees = transactions.reduce((sum, tx) => {
            ...
            if ((tx.type === 'BATCH' || tx.type === 'CONVERSATION_BATCH') && tx.payload && Array.isArray(tx.payload.transactions)) {
                ...
                const innerFees = tx.payload.transactions.reduce((innerSum, innerTx) => innerSum + (innerTx.amount || 0), 0);
                txFee += innerFees;
            }
            return sum + txFee;
        }, 0);
    ...
    validateBlock(block, previousBlock) {
        ...
        if (this.validatorPool && block.index > 0) { // Genesis block might be special
            ...
            if (validator.public_key.length < 64) {
                ...
                if (!KeyManager_1.KeyManager.verify(signableData, block.signature, validator.public_key)) {
                ...
                for (const sig of block.signatures) {
                    ...
                    for (const v of activeValidators) {
                        if (!usedValidators.has(v.validator_id)) {
                            // Try verify
                            if (KeyManager_1.KeyManager.verify(hashToVerify, sig, v.public_key)) {
    ...
    calculateStateRoot(transactions) {
        ...
        return KeyManager_1.KeyManager.hash(stateString);
    }
    /**
    ...
    restoreChain(blocks) {
        ...
        try {
            for (const block of blocks) {
                ...
                const res = this.applyTransactions(block.transactions, block.validator_id, block.index, block.node_wallet);
                if (!res.success) {
     ...
     */
    applyTransactions(transactions, validatorId, // Optional for backward compatibility or simulation
    blockIndex, // Block index to check if genesis
    nodeWallet // Optional node wallet for fee distribution
    ) {
        for (const tx of transactions) {
     ...
     */
    applyTransactionToState(tx, state, validatorId, blockIndex, // Block index to check if genesis
    nodeWallet, // Optional node wallet for fee distribution
    shouldCache = true // New parameter
    ) {
        var _a;
        ...
        if (tx.type !== Transaction_1.TransactionType.REWARD) {
            ...
            const txModel = new Transaction_1.TransactionModel(tx);
            const signableData = txModel.getSignableData();
            // Verify
            if (!KeyManager_1.KeyManager.verify(signableData, tx.sender_signature, verifyKey)) {
        ...
        switch (tx.type) {
            ...
            case 'CONVERSATION_BATCH':
                ...
                for (const innerTx of tx.payload.transactions) {
                    try {
                        ...
                        if (!KeyManager_1.KeyManager.verify(signableData, innerTx.signature, verifyKey)) {
                            // Fallback: Check if innerTx has sender_public_key attached
                            if (innerTx.sender_public_key && KeyManager_1.KeyManager.verify(signableData, innerTx.signature, innerTx.sender_public_key)) {
                        ...
                        const pPool = TokenConfig_1.TOKEN_CONFIG.FEE_TO_POOL_PERCENT / 100; // 0.37
                        const pDev = TokenConfig_1.TOKEN_CONFIG.FEE_TO_DEV_PERCENT / 100; // 0.08
                        const pRecycle = TokenConfig_1.TOKEN_CONFIG.FEE_TO_RECYCLE_PERCENT / 100; // 0.15
                        ...
                        const poolAddr = TokenConfig_1.TREASURY_ADDRESSES.validator_pool;
                        ...
                        const devAddr = TokenConfig_1.TREASURY_ADDRESSES.development;
                        ...
                        const recycleAddr = TokenConfig_1.TREASURY_ADDRESSES.recycle;
                        ...
                        let primaryAddr = TokenConfig_1.TREASURY_ADDRESSES.main;
            ...
            case 'TRANSFER':
                ...
                const priority = ((_a = tx.payload) === null || _a === void 0 ? void 0 : _a.priority) || 'STANDARD';
                ...
                if (tx.fee > 0) {
                    const fee = tx.fee;
                    const pPool = TokenConfig_1.TOKEN_CONFIG.FEE_TO_POOL_PERCENT / 100;
                    const pDev = TokenConfig_1.TOKEN_CONFIG.FEE_TO_DEV_PERCENT / 100;
                    const pRecycle = TokenConfig_1.TOKEN_CONFIG.FEE_TO_RECYCLE_PERCENT / 100;
                    const poolShare = Math.floor(fee * pPool);
                    const devShare = Math.floor(fee * pDev);
                    const recycleShare = Math.floor(fee * pRecycle);
                    ...
                    const primaryShare = fee - poolShare - devShare - recycleShare;
                    const poolAcc = state.get(TokenConfig_1.TREASURY_ADDRESSES.validator_pool) || { address: TokenConfig_1.TREASURY_ADDRESSES.validator_pool, balance: 0, nonce: 0 };
                    poolAcc.balance += poolShare;
                    state.set(TokenConfig_1.TREASURY_ADDRESSES.validator_pool, poolAcc);
                    const devAcc = state.get(TokenConfig_1.TREASURY_ADDRESSES.development) || { address: TokenConfig_1.TREASURY_ADDRESSES.development, balance: 0, nonce: 0 };
                    devAcc.balance += devShare;
                    state.set(TokenConfig_1.TREASURY_ADDRESSES.development, devAcc);
                    const recycleAcc = state.get(TokenConfig_1.TREASURY_ADDRESSES.recycle) || { address: TokenConfig_1.TREASURY_ADDRESSES.recycle, balance: 0, nonce: 0 };
                    recycleAcc.balance += recycleShare;
                    state.set(TokenConfig_1.TREASURY_ADDRESSES.recycle, recycleAcc);
                    const mainAcc = state.get(TokenConfig_1.TREASURY_ADDRESSES.main) || { address: TokenConfig_1.TREASURY_ADDRESSES.main, balance: 0, nonce: 0 };
                    mainAcc.balance += primaryShare;
                    state.set(TokenConfig_1.TREASURY_ADDRESSES.main, mainAcc);
                }
            ...
            case 'PROFILE_UPDATE':
                if (tx.payload) {
                    ...
                    if (tx.payload.public_key)
                        fromAccount.public_key = tx.payload.public_key;
                    if (tx.payload.encryption_public_key)
                        fromAccount.encryption_public_key = tx.payload.encryption_public_key;
                    if (tx.payload.nickname)
                        fromAccount.nickname = tx.payload.nickname;
                }
     ...
     */
    static fromJSON(data, genesisValidatorId, validatorPool) {
        const blockchain = new Blockchain(genesisValidatorId, validatorPool);
        blockchain.chain = data.map((blockData) => new Block_1.Block(blockData));
     ...
     */
    calculateTransferFee(toAccount, amount, priority = 'STANDARD') {
        ...
        const fees = TokenConfig_1.TOKEN_CONFIG.DYNAMIC_TRANSFER_FEES;
    ...
    processChainSegment(segment) {
        ...
        if (prevIndex >= 0) {
            ...
            if (!prevBlock)
                return { success: false, error: 'Critical: Previous block missing for link' };
